...
namespace UNIR.TFE.Polyrepo.Division.Module.Test
{
    public class DivisionAppServiceStableTests
    {
        private readonly DivisionAppService _sut;

        // EXACTAMENTE 10 000 casos en el dataset masivo
        private const int BULK_CASES_COUNT = 10_000;

        public DivisionAppServiceStableTests()
        {
            _sut = new DivisionAppService();
        }
        ...
        [Fact]
        public void Key_ShouldReturn_Div_WhenRequested()
        {
            const string expectedKey = "div"; // cambia si tu servicio usa otra clave
            Assert.Equal(expectedKey, _sut.Key);
        }

        [Fact]
        public void Execute_ShouldThrow_OnDivideByZero()
        {
            Assert.Throws<DivideByZeroException>(() => _sut.Execute(1, 0));
            Assert.Throws<DivideByZeroException>(() => _sut.Execute(0, 0));
        }
        ...
        [InlineData(-5, -2.5, 2)]
        [InlineData(0, 7, 0)]
        public void Execute_ShouldReturn_CorrectQuotient_ForGivenOperands(decimal a, decimal b, decimal expected)
        {
        ...
        [InlineData(1)]
        public void Execute_Identities(decimal a)
        {
            Assert.Equal(a, _sut.Execute(a, 1)); // a/1 = a
            Assert.Equal(0, _sut.Execute(0, a)); // 0/a = 0
            if (a != 0) Assert.Equal(1, _sut.Execute(a, a)); // a/a = 1
        }
        ...
        public void Execute_BulkRandomizedDataset_ReturnsCorrectQuotient(decimal a, decimal b, decimal expected)
        {
